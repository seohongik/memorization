...
<h2>발행인 에세이 페이지 입니다.</h2>

<form action="/board/officialBoardList/essay" method="get">
	...
	<ul class='pagination' style="display: flex; flex-wrap: nowrap; justify-content: center;">
			...
			<li  style='list-style: none;'  class='paginate_button'   >
				<c:choose>
					<c:when test="${num == forList.pageMaker.pageNum}">
						...
						<input name="list" value="essay" style="display: none"/>
					</c:when>
					<c:otherwise>
						<a class="blockLink" style="padding: 0 10px;" href="/board/officialBoardList/essay?pageNum=${num}&amount=${forList.pageMaker.amount }">${num}</a>
					</c:otherwise>
				</c:choose>
			</li>
		</c:forEach>
		<c:if test="${forList.pageMaker.next}">
			<li style="list-style: none" class="paginate_button next">
				<a href="/board/officialBoardList/essay?pageNum=${forList.pageMaker.endPage +1 }">Next</a>
			</li>
		</c:if>
	</ul>
</div>
