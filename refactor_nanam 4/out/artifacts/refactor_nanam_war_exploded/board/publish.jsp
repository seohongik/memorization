...
<div class="basebar">
	<ul>
		...
		<li><a href="#">출판</a></li>
		<li>></li>
		<li><a href="#">언론이 본 나남</a></li>
	</ul>
</div>
<h2>언론이 본 나남 페이지 입니다.</h2>

<form action="/board/officialBoardList/nanam" method="get">
		...
		<ul class='pagination' style="display: flex; flex-wrap: nowrap; justify-content: center;">
			...
			<c:if test="${forList.pageMaker.prev}">
				<li style='list-style: none' class='paginate_button previous'>
					<a href='/board/officialBoardList/essay?pageNum=${forList.pageMaker.startPage -1 }'>Previous</a>
				</li>
			</c:if>
				...
				<li  style='list-style: none'  class='paginate_button'   >
					<c:choose>
						<c:when test="${num == forList.pageMaker.pageNum}">
							...
							<input name="list" value="nanam" style="display: none"/>
						</c:when>
						<c:otherwise>
							<a class="blockLink" style="padding: 0 10px;" href="/board/officialBoardList/nanam?pageNum=${num}&amount=${forList.pageMaker.amount }">${num}</a>
						</c:otherwise>
					</c:choose>
				</li>
			</c:forEach>
			<c:if test="${forList.pageMaker.next}">
				<li style="list-style: none" class="paginate_button next">
					<a href="/board/officialBoardList/nanam?pageNum=${forList.pageMaker.endPage +1 }">Next</a>
				</li>
			</c:if>
		</ul>
	</div>
